...
```
            ...
            M      N                             A        I
          O   G  K   D                        W   R    U    S
         -  + Q Z Y C X B J P ^ L  2  F V H   (depth 4)
        0 9 / 8 ` 7 ~ 6 ' ? 1  /  4  * 3 5   (depth 5)
```
...
### 8.3 Vòng lặp chính (Main Loop)
 
```c
const int table[12] = {0x32,0x30,0x32,0x30,0x30,0x33,
                        0x33,0x31,0x20,0x73,0x4F,0x43};
// = "20200331 sOC"
...
int main() {
    ...
    SendCommand(0x80);   // Line 1
    for (int i = 0; i < 12; i++)
        SendData(table[i]);
    SendCommand(0xC0);   // Line 2
 
    // --- Vòng lặp chính ---
    while (1) {
        unsigned int btn        = IORD(BUTTON_BASE, 0);
 
        // Ghi trạng thái KEY[1:0] vào Morse IP
        IOWR(MORSE_HW_0_BASE, 0, (btn & 0x3));
 
        // Đọc kết quả từ Morse IP
        unsigned int reg_val    = IORD(MORSE_HW_0_BASE, 1); // morse_code
        unsigned int bit_val    = IORD(MORSE_HW_0_BASE, 2); // length
        unsigned int ascii_code = IORD(MORSE_HW_0_BASE, 3); // ASCII out
        unsigned int mode       = IORD(MORSE_HW_0_BASE, 0); // current mode
 
        // KEY[2] (bit 2) = 0 → ghi ký tự lên LCD
        if (mode == 0 || mode == 1) {
            if (((btn & 0b100) >> 2) == 0) {
                SendData(ascii_code);
                printf("Mode: 0x%X | morse_code: 0x%X | "
                       "length: 0x%X | ASCII: 0x%X\n\n",
                       mode, reg_val, bit_val, ascii_code);
            }
        }
    }
}
...
- ⚠️ FSM dùng blocking assignment (`=`) trong `always @(posedge clk)` — tiềm ẩn rủi ro synthesis.
...
- 🔧 Chuyển toàn bộ FSM sang **non-blocking assignment** (`<=`).
---
## Phụ lục B — Giao tiếp giữa LCD 16×2 và Board DE10 Standard
 
### B.1 Tổng quan LCD 16×2 (HD44780)
...
### B.2 Sơ đồ nối dây LCD với GPIO Board DE10
...
### B.3 Ánh xạ bit GPIO → tín hiệu LCD
...
### B.4 Giản đồ thời gian (Timing Diagram)
...
### B.5 Hàm điều khiển LCD trong C
...
### B.6 Trình tự khởi động LCD đầy đủ
...
### B.7 Bảng lệnh HD44780 được sử dụng
...
### B.8 Luồng hoạt động tổng thể LCD trong hệ thống
...
### B.9 Ví dụ thực tế: Hiển thị ký tự 'G' (0x47) lên dòng 2
